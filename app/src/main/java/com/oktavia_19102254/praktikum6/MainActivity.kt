...
import adapter.CardViewMyDataAdapter
import adapter.GridMyDataAdapter
import adapter.ListMyDataAdapter
...
fun getListMyDatas(): ArrayList<MyData> {
    ...
    for (position in dataName.indices) {
        val myData = MyData(
            ...
            dataPhoto[position] )
        listMyData.add(myData) } return listMyData }

    }
}
private fun showRecyclerList() {
    rv_mydata.layoutManager = LinearLayoutManager(this)
    val listMyDataAdapter = ListMyDataAdapter(private val list = ArrayList<MyData>())
    rv_mydata.adapter = listMyDataAdapter }
rv_mydata.setHasFixedSize(true) list.addAll(getListMyDatas()) showRecyclerList()
